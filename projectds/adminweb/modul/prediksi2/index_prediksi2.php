...
<form action="index.php" method="GET">
    <input type="hidden" name="mod" value="prediksi2">
...
<?php
if (isset($_GET['obat']) && isset($_GET['mod']) && $_GET['mod'] == 'prediksi2') {
    $selected_obat = $_GET['obat'];

    // Query untuk mengambil data jumlah pemesanan berdasarkan obat yang dipilih
    $conn = new mysqli($servername, $username, $password, $dbname);
    $sql = "SELECT * FROM pemesanan p JOIN obat o ON o.id_obat = p.id_obat WHERE o.nama = '$selected_obat' ORDER BY periode ASC";
    $result = $conn->query($sql);
    ...
    echo "<h2>Data Pemesanan dan EMA (Exponential Moving Average) untuk Obat $selected_obat</h2>";
    echo "<table>";
    echo "<tr><th>Periode</th><th>Jumlah Pemesanan</th><th>EMA</th><th>Percentage Error</th></tr>";
        ...
        const config = {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Grafik Jumlah pemesanan dan EMA untuk Obat <?php echo $selected_obat?>'
                    }
                }
            },
        };
